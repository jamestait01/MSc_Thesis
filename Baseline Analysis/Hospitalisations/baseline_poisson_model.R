data <- readRDS("~/baseline_models/model_3a/merged_df_baseline.rds")
...
cardiac_model <- glm(n_cardiac_unique ~ Sex + `Age group` + BMI + `Smoking status` +  `Weekly alcohol intake` +
                       `Index of Multiple Deprivation` + `Number of comorbidities` +
                       `Medication combination`,family="poisson",data=data)
...
ggplot(data = table, aes(x = Level, y = OR, color = model_type)) +
