...
poisson_model <- glm(n_hospitalisations_unique ~ Sex + `Age group` + BMI + `Smoking status` +  `Weekly alcohol intake` +
                       `Index of Multiple Deprivation` + `Number of comorbidities` +
  `Medication status`,family="poisson",data=data)

cardiac_poisson <- glm(n_cardiac_unique ~ Sex + `Age group` + BMI + `Smoking status` +  `Weekly alcohol intake` +
                         `Index of Multiple Deprivation` + `Number of comorbidities` +
                         `Medication status`,family="poisson",data=data)
...
poisson_meds <- glm(n_hospitalisations_unique ~ Sex + `Age group` + BMI + `Smoking status` +  `Weekly alcohol intake` +
                      `Index of Multiple Deprivation` + `Number of comorbidities` +
                       `Medication combination`,family="poisson",data=stable_meds)

cardiac_meds <- glm(n_cardiac_unique ~ Sex + `Age group` + BMI + `Smoking status` +  `Weekly alcohol intake` +
                      `Index of Multiple Deprivation` + `Number of comorbidities` +
                      `Medication combination`,family="poisson",data=stable_meds)
...
poisson_cardiac_tbl <- cardiac_meds %>%
  tbl_regression(exponentiate=T) %>%
  bold_labels()
...
#get rid of reference levels so the points aren't displayed on the graph but the labels still are.
