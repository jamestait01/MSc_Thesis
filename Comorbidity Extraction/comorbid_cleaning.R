...
#this gives all of the icd codes for people in the biobank, along with their date
icd10_data <- readRDS("extraction_and_recoding/outputs/ukb_extracted.rds")

#load all prevalent and incident cases required for analysis
eids <- readRDS("Data/follow_up1/merged_data.rds")
#keep only eids
eids <- unique(eids$eid)

#subset icd file to only keep people we're interested in.
...
test <- icd_baseline[1:100,]

library(tidyverse)

icd_diag <- select(test, eid, date_recr, starts_with("icd10_diag"))

icd_date_diag <- select(test,eid,starts_with("icd10_date_diag"))
...
icd_test <- bind_cols(icd_diag_t,icd_date_diag_t)

View(icd_test)

#So this code works more or less. Let's apply it to the big dataset.
