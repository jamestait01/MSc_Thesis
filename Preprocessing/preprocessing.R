...
saveRDS(original_data,"preprocessing/original_data.rds")
...
#saves original data before additional preprocessing
saveRDS(original_data,"preprocessing/original_data.rds")
...
#repeats the above for pack years
#compares each instance of smoking stauts to see if the values are the same for each person and instance
table(apply(data_cleaned2[, c("pack_years.0.0", "pack_years.1.0", "pack_years.2.0", 
                              "pack_years.3.0")],1, function(x) all(x == x[1], na.rm = TRUE)))
#488471 people have the same pack year for each instance, 13549 do not (2.5%), so just take the first instance

#drop pack year follow-up instances
data_cleaned2$pack_years.1.0 <- NULL
data_cleaned2$pack_years.2.0 <- NULL
data_cleaned2$pack_years.3.0 <- NULL
...
#if someone has never smoked, put 0 pack years
data_cleaned2$pack_years.0.0 <- ifelse(data_cleaned2$smoking_status.0.0=="Never",0,data_cleaned2$pack_years.0.0)
