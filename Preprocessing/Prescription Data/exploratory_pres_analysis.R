...
saveRDS(cases,"/rds/general/project/hda-22-23/live/Summer_projects/jt1019/Data/merged_df.rds")

#create separate df's for everyone with no meds and everyone with meds
no_meds <- cases[is.na(cases$drug_name), ]

meds <- cases[!is.na(cases$drug_name),]

n_distinct(meds$eid)
#There are 8498405 rows yet only 87000 unique ids. There are a lot of duplicated rows where the
...
#repeat for unique_meds df

# Calculate the prevalence of each factor level
factor_counts <- table(unique_meds$ethnicity.0.0)
factor_prevalence <- prop.table(factor_counts)
...
# Reclassify levels as "Other"
unique_meds$ethnicity.0.0 <- factor(unique_meds$ethnicity.0.0)
levels(unique_meds$ethnicity.0.0)[levels(unique_meds$ethnicity.0.0) %in% other_levels] <- "Other"

summary(unique_meds$ethnicity.0.0)
