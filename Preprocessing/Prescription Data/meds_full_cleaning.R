...
#loads variables dataset to add idx_multdep and alcohol intake info
...
#remove other instances with mostly NA's
variables$alcohol_intake.1.0 <- NULL
variables$alcohol_intake.2.0 <- NULL
variables$alcohol_intake.3.0 <- NULL

#combine idx_multdep for england, scotland and wales into one column
...
variables$idx_multdep <- ifelse(!is.na(variables$idx_multdep_e.0.0), variables$idx_multdep_e.0.0,
                         ifelse(!is.na(variables$idx_multdep_w.0.0), variables$idx_multdep_w.0.0,
                                ifelse(!is.na(variables$idx_multdep_s.0.0), variables$idx_multdep_s.0.0, NA)))

#remove now redundant idx_multdep_e/s/w columns
variables$idx_multdep_e.0.0 <- NULL
variables$idx_multdep_s.0.0 <- NULL
variables$idx_multdep_w.0.0 <- NULL

#merge dfs together by eid
meds <- merge(meds,variables,by="eid")
...
#saveRDS(meds,"Data/cases_meds_final2.rds")
######################################################################
meds <- readRDS("Data/cases_meds_final2.rds")
...
test <- meds[,c("eid","issue_date","drug_name","med_cat", "quantity","first_word","dosage","unit")]
